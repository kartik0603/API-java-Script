...
const getdata = async () => {
    ...
    document.getElementById("search").addEventListener("submit", (e) => {
        e.preventDefault();
        let value = document.getElementById("find").value.toLowerCase();
        let temp = res.filter((ele) => ele.category.toLowerCase().includes(value));
        mapper(temp);
    });
}
...
const mapper = (data) => {
    ...
    data.forEach((item) => {
        ...
        let rate = createTag("h6", `Ratings: <i class="fa-solid fa-star"></i> ${item.rating.rate}`);
